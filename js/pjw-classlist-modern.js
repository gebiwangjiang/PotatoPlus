function ClassListPlugin() {
  ...
  */
    ...
    getHTML(data, attr, options = {}) {
      ...
      function getClassInfo(content) {
        ...
        for (var item of content) {
          if ("val" in item && (item.val == null || item.val == "")) continue;
          if ("key" in item) {
            if (item.key == "课程编号") {
              item.val = `<span class="pjw-class-course-number cv-jxb-detail" data-number="${item.val}">${item.val}</span>`;
            }
        ...
        if (!data.status) return "";
          ...
          <div class="pjw-class-comment-button alter">
            <div class="pjw-class-comment-button__container">
              <div class="material-icons-round pjw-class-comment-icon">bookmark_border</div>
  ...
  window.PJWClassList = class {
    ...
    constructor(parent, modules = ["avail", "frozen"]) {
